...
if m.status == GRB.OPTIMAL:
    ...
    for l in L:
        ...
        if constr and abs(constr.Pi) >= 0: # Filter to show non-zero or just first 5
             print(f"{l:<10} | {capacity[l]:<10} | {constr.Pi:.2f}")
        
        # Limit print to first 5 for readability (remove break to see all)
        count += 1
        if count >= 5: break
    ...
    for p in demand_data:
        ...
        # Limit print to first 5 for readability
        count += 1
        if count >= 5: break
